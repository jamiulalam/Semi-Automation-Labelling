...
def get_estimated_angle(template_data,moving_data,angle_step_size,min_size,num_of_split=7, worker_allocation=0.80, \
    ...
    for i in range(split_size):
        ...
        if last_num-1<(i*vol_size)+1:
            index_list.append(random.randint(last_num-1,(i*vol_size)+1))
        else:
...
def register_3D_volume(reference_vol_path, input_vol_path, registration_type=1, chunk_size = 30, \
    ...
    \n
    \n registration_type : 1- fast registration 2- slow but more accurate 
    ...
    print(msg_reg)
    return ref_reg, volume_reg, psnr, rotation_angle, msg_reg, (toc/60.0), worker_to_use
